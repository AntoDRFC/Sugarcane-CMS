...
function isWork()
{
	...
	if (!is_dir($cfg['root'])) {
		...
		if (mkdirs($cfg['root'], $cfg['chmod']['folder'])) {
			...
			echo 'Failed created, You need to create the folder manually, or set the right permissions<br />';
			exit;
		}
	} elseif (!is_writable($cfg['root'])) {
		echo 'No write access to folder: ' . $cfg['root'], '<br />';
	} else {
		echo '<font color=green>Root directory found, ready to proceed<br /></font>';
	}
	echo '<hr />';
	...
	for ($i=-1;++$i<4;) {
		...
		if (!is_dir($d)) {
			...
			if (mkdirs($d, $cfg['chmod']['folder'])) {
				...
				echo 'Failed created, You need to set the right permissions in Plesk<br />';
				exit;
			}
		} elseif (!is_writable($d)) {
			echo 'No write access to folder: ' . $d, '<br />';
		} else {
